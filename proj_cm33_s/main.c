...
#define CM33_RESET_HANDLER_STORE    (CM33_SP_STORE + 4)

/* Secure interrupt priority */
#define S_ISR_PRIORITY              3u
...
/**
 * @brief S GPIO Handler.
 *
 * This is the S GPIO interrupt handler. The handler toggles the S LED.
 *
 * @return Void.
 */
void s_gpio_interrupt_handler(void);
...
void s_gpio_interrupt_handler(void)
{
    ...
    if(CM33_NS_PC_VALUE == Cy_Ms_Ctl_GetSavedPC(CPUSS_MS_ID_CM33_0))
    {
        mscStatus = Cy_Ms_Ctl_SetSavedPC(CPUSS_MS_ID_CM33_0, CM33_NS_PC_VALUE);
        if(CY_RSLT_SUCCESS == mscStatus)
        {
            mscStatus = Cy_Ms_Ctl_SetActivePC(CPUSS_MS_ID_CM33_0, CM33_NS_PC_VALUE);
            if(CY_RSLT_SUCCESS != mscStatus)
            {
 ...
 *    1. Configures secure GPIO and enables IRQ
...
int main(void)
{
    ...
    /* Set secure interrupt */
    NVIC_SetVector(CYBSP_USER_BTN1_IRQ, (uint32_t)s_gpio_interrupt_handler);

    NVIC_SetPriority(CYBSP_USER_BTN1_IRQ, S_ISR_PRIORITY);

    NVIC_EnableIRQ(CYBSP_USER_BTN1_IRQ);

    /* Configure SAU for SRAM0, Flash and Peri MMIO */
    TZ_SAU_Setup();
